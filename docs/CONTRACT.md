...
2. Вперемешку, по мере готовности: `source_status` (итоговый статус источника), `photo`, `summary`, `stage`.
   `stage` — что сборка делает сейчас, для экрана загрузки: `{"stage": "sources"}` → `{"stage": "gap_fill", "categories": ["libraries"]}` (только если есть пустые категории) → `{"stage": "vision", "checked": 40, "total": 200}` (повторяется после каждой пачки, `total` null до подсчёта) → `{"stage": "finalizing"}`. Из кэша `stage` не приходит. Клиент игнорирует незнакомые события.
...
- Свежесть меняет только порядок и подпись. Она никогда не повышает и не понижает `tier`. Порядок в `done.photo_ids`:
  сначала `verified`/`likely` по `confidence` + бонус свежести (2024+: +20, 2020–2023: +10, older/date_unknown: −5, historic: −10), затем `unconfirmed`;
  свежее `likely` может стоять выше старого `verified`. Затем порядок витрины: до 4 фото с `highlight=true` (главный корпус, главные ворота, панорама; кадры Mapillary последними), потом надёжные фото по очереди из категорий campus → dorms → classrooms → libraries → city без двух фото одной страницы подряд, потом `unconfirmed`. Клиент показывает этот порядок как «рекомендуемые». `Photo.highlight` ставит Claude или текст (main building, main gate, панорама).
...
class SourceQuery:
    ...
    category_targets: dict[str, int] | None                 # потолок надёжных фото на категорию из PHOTO_TARGETS; по умолчанию {} — без потолка
